...
class UMLDiagram extends Component {
  ...
  shorten(filename) {
    return filename.replace(this.gopath, "");
  }
  ...
  render() {
    ...
    let getStructRef = (pkg, file, struct) =>
      getPackageRef(pkg) + "/" + getFileRef(pkg, file) + "/" + struct.name;
    ...
    let edges = (() => {
      ...
      setInterval(() => {
        ...
        for (let { ref, data: { from, to } } of edges) {
          ...
          $(this.refs[ref]).attr({
            d: `M ${start.x} ${start.y} q ${anchorX} ${anchorY} ${end.x -
    ...
    return (
      ...
      >
        <div className="diagram" style={transform}>
  ...
  deselect(e) {
    if (
      distFromParent(e.target, "package") === null &&
  ...
  onFileClick(path, e) {
    // TODO Make a finer filter (don't include child clicks from structs)
    if (
      distFromParent(e.target, "file") < distFromParent(e.target, "package")
    ) {
      // select file
